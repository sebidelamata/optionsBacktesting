...
We only intend to build this table once, for the following days we will append to this table. However, we do want to standardize this procedure. The script to create a table will also allow overwriting of existing data. We do not need to worry about defining the shema (will be saved in `public`) nor whether a column is `VARCHAR(9)` or `VARCHAR(10)`. `R` will handle all of this for us.
...
The second part of our `cron` job tells our Raspberry Pi what to do at that time. In this case, first `cd /home/sebi/optionsBacktesting` change directory, then `Rscript trading_day_scheduler.R` run the `R` script `trading_day_scheduler.R` (to see if the market is open and add data to the PostgreSQL database if it is. The final part `> log.txt 2>&1` send any terminal information to a log file we created.
...
Next let's plot our data as a bar chart so we can really see the differences. I used the `Rushmore1` palette from the `wesanderson` package because I am a shameless millennial. Until the next post, "I'll just go out the back window."
